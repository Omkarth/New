...
public class Request_Apply_Electrician extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_request__apply__electrician);

        mAuth = FirebaseAuth.getInstance();
        servicer_id = mAuth.getCurrentUser().getUid();
        acceptref = FirebaseDatabase.getInstance().getReference().child("Accepted Requests").child("Electrician");
        acceptreddelete = FirebaseDatabase.getInstance().getReference().child("Request Pickup").child("Electrician Requested For Permanent");
        userref = FirebaseDatabase.getInstance().getReference().child("Registered Users");
        chefref = FirebaseDatabase.getInstance().getReference().child("Request Pickup").child("/Electrician Requested For Permanent");
        recyclerView = (RecyclerView) findViewById(R.id.recyclerview2);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));
        progressDialog = new ProgressDialog(Request_Apply_Electrician.this);
    ...
    protected void onStart() {
        ...
        FirebaseRecyclerOptions<List_requested_apply_Electrician> options =
                new FirebaseRecyclerOptions.Builder<List_requested_apply_Electrician>()
                        .setQuery(chefref, List_requested_apply_Electrician.class)
                        .build();
        FirebaseRecyclerAdapter<List_requested_apply_Electrician, Request_Apply_Electrician.FindFirndViewHolder> adapter = new FirebaseRecyclerAdapter<List_requested_apply_Electrician, Request_Apply_Electrician.FindFirndViewHolder>(options) {


            @Override
            public void onBindViewHolder(final Request_Apply_Electrician.FindFirndViewHolder viewHolder, final int position, List_requested_apply_Electrician model) {
                viewHolder.t.setText(model.getMaid());
                ...
                viewHolder.accept.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(final View v) {
                        ...
                        AlertDialog.Builder alertDialog = new AlertDialog.Builder(Request_Apply_Electrician.this);
                                ...
                                .addOnCompleteListener(new OnCompleteListener<Void>() {
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            Toast.makeText(Request_Apply_Electrician.this, "Rejected", Toast.LENGTH_SHORT).show();
                                        }
                                    }
                                });
                    }
            ...
            @Override
            public Request_Apply_Electrician.FindFirndViewHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
                View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.list_request_apply_items_chef, viewGroup, false);
                Request_Apply_Electrician.FindFirndViewHolder viewHolder = new Request_Apply_Electrician.FindFirndViewHolder(view);
                return viewHolder;
            }
        };
